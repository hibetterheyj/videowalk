...
def test(loader, model, args):
    ...
    for vid_idx, (imgs, imgs_orig, lbls, lbls_orig, lbl_map, meta) in enumerate(loader):
        ...
        seq_name = meta['folder_path'][0].split('/')[-1]
        print('******* Vid %s - %s (%s frames) *******' % (vid_idx, seq_name, N))
        with torch.no_grad():
            t00 = time.time()
            ...
            for t in range(key_indices.shape[0]):
                ...
                if 'VIP' in args.filelist:
                    ...
                    outpath = os.path.join(args.save_path, seq_name)
                    os.makedirs(outpath, exist_ok=True)
                    img_prefix = os.path.join(outpath, str(vid_idx) + '_' + str(t))
